...
The dashboard is a local presentation of frozen Phase 5-9 artifacts. The browser fetches only the same-origin public JSON and static assets. It filters descriptive views; it does not refit statistics or models.
...
| All local sensor-day rows (includes unqualified dates) | 182 |
| Qualified full sensor-days | 172 |
...
This walkthrough verifies the public bundle and SUCCESS.json bytes, then compares their hash declarations. It does not re-read each underlying payload; run the dashboard builder below for those checks. All three Phase 7-9 statuses remain `limited_diagnostic`.
